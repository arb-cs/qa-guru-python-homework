    ...
    def test_fill_entire_form(self):
        ...
        form_title = self.wait.until(
            EC.visibility_of_element_located(
                (By.XPATH, "/html/body/main/section/h1")
            )
        )
        ...
        form_sub_title = self.wait.until(
            EC.visibility_of_element_located(
                (By.XPATH, "/html/body/main/section/div/p")
            )
        )
        ...
        wait.until(
            EC.visibility_of_element_located(
                (By.XPATH, "//*[contains(text(), 'Level up your automation')]")
            )
        )
        # Находим и кликаем по кнопке закрытия (крестику) модального окна
        close_banner_btn = wait.until(
            EC.element_to_be_clickable(
                (By.XPATH, """//*[@id="fixedban"]/div/div/button""")
            )
        )
        ...
        self.wait.until(
            EC.visibility_of_element_located(
                (By.CLASS_NAME, "react-datepicker__month-container")
            )
        )
        ...
        month_select.find_element(
            By.XPATH, "//option[@value='11']"
        ).click()  # 11 — это Декабрь
        ...
        hobby_music = driver.find_element(
        ...
        state_option = self.wait.until(
            EC.element_to_be_clickable(
                (By.XPATH, """//*[@id="stateCity-wrapper"]/div[1]""")
            )
        )
        state_option.click()
        ...
        )  # Надежный клик через JS без перекрытий
        ...
        self.assertIn(
            "25 Dec 1995", result_table.text
        )  # форма вывода даты может меняться от настроек
