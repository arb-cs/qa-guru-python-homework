...
class AutomationPracticeFormPO:
    ...
    def _close_commercial_banner(self):
        banner_button = self.wait.until(
            ec.element_to_be_clickable(self.BANNER_BUTTON)
        )
        banner_button.click()
    ...
    def _select_birth_day(self, date: Tuple[str, str, str]):
        ...
        self.driver.find_element(
            By.CSS_SELECTOR, f".react-datepicker__day--0{date[2]}[tabindex='0']"
        ).click()

    def _upload_file(self, file_path):
        self.driver.find_element(*self.UPLOAD_PICTURE_BUTTON).send_keys(
            file_path
        )
    ...
    def _fill_subject(self, *subjects):
        ...
        self.driver.execute_script(
            "arguments[0].scrollIntoView();", subjects_input
        )
    ...
    def _fill_current_address(self, current_address):
        current_address_field = self.driver.find_element(
            *self.CURRENT_ADDRESS_FIELD
        )
    ...
    def _select_state(self, state):
        ...
        state_dropdown = self.wait.until(
            ec.element_to_be_clickable(
                (
                    By.XPATH,
                    f"//div[@class='state-city-option'][text()='{state}']",
                )
            )
        )
    ...
    def _select_city(self, city):
        ...
        city_dropdown = self.wait.until(
            ec.element_to_be_clickable(
                (
                    By.XPATH,
                    f"//div[@class='state-city-option'][text()='{city}']",
                )
            )
        )
        city_dropdown.click()
    ...
    def fill_in_form(
        ...
        practice_form_title = self.driver.find_element(
            *self.PRACTICE_FORM_TITLE
        )
        assert (
