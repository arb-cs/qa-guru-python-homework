...
class TextBoxPage:
    ...
    def open(self):
        ...
        return (
            self  # NOTE: for Fluent Interface Implementation (just an example)
        )
    ...
    def fill_form(self, name=None, email=None, cur_addr=None, perm_addr=None):
        ...
        if perm_addr is not None:
            self.driver.find_element(*self.PERMANENT_ADDRESS).send_keys(
                perm_addr
            )
    ...
    def get_output_data(self):
        ...
        name = (
            self.driver.find_element(*self.OUTPUT_NAME)
            .text.replace("Name:", "")
            .strip()
        )
        email = (
            self.driver.find_element(*self.OUTPUT_EMAIL)
            .text.replace("Email:", "")
            .strip()
        )
        cur_addr = (
            self.driver.find_element(*self.OUTPUT_CUR_ADDR)
            .text.replace("Current Address :", "")
            .strip()
        )
        perm_addr = (
            self.driver.find_element(*self.OUTPUT_PERM_ADDR)
