...
def test_positive_all_fields(registration_page, temp_test_file):
    ...
    registration_page.set_state(state_name)
    registration_page.select_city(city_name)
    registration_page.click_submit_button()
    registration_page.get_result_form()

    result_text = registration_page.get_result_form()
    expected_date = registration_page.get_expected_birth_date_text(
        day, month, year
    )
    ...
    assert gender in result_text, f"Пол '{gender}' не найден!"
    assert (
        expected_date in result_text
    ), f"Дата рождения '{expected_date}' не найдена!"
    assert (
        expected_subjects in result_text
    ), f"Предметы '{expected_subjects}' не найдены!"
    assert (
        expected_hobbies in result_text
    ), f"Хобби '{expected_hobbies}' не найдены!"
    assert file_name in result_text, f"Файл '{file_name}' не найден!"
    assert (
        current_address in result_text
    ), f"Адрес '{current_address}' не найден!"
