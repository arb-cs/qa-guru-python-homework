...
class RegistrationPage(BasePage):
    ...
    SUBJECTS_MENU_OPTION = (
        By.CSS_SELECTOR,
        "div[class='subjects-auto-complete__option']",
    )
    # Локаторы гендер, gender_value = gender-radio-1 (2,3)
    GENDER_RADIO_LOCATOR = (
        By.CSS_SELECTOR,
        "label:has(input[value='{gender_value}'])",
    )
    # Локаторы хобби
    ...
    HOBBY_LABEL_LOCATOR = (
        By.XPATH,
        "//label[contains(text(), '{hobby_name}')]",
    )
    ...
    def select_gender(self, gender: str):
        ...
        if gender_value not in ["Male", "Female", "Other"]:
            raise ValueError(
                f"Неверный гендер: '{gender}'. Допустимые варианты: Male, Female, Other"
            )
    ...
    def get_expected_birth_date_text(
    ...
    def select_state(self, state_name: str):
        ...
        state_option = (
            By.XPATH,
            f"//div[@class='state-city-option' and text()='{state_name}']",
        )
    ...
    def select_city(self, city_name: str):
        ...
        city_option = (
            By.XPATH,
            f"//div[@class='state-city-option' and text()='{city_name}']",
        )
